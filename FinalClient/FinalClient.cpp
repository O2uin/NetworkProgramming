...
static FILESEND_MSG  g_filesendmsg;

/*파일 열기 관련 변수*/
HWND hButtonOpenFileDialog;			//열기 대화상자 핸들
HWND hEditFileToBeOpened;			//파일 경로, 이름 가져오는 헨들
OPENFILENAME ofn;					//열기 대화상자 초기화 변수
const UINT nFileNameMaxLen = 512;	//szFileName 문자열 최대 길이
WCHAR szFileName[nFileNameMaxLen];	//파일 경로 및 이름 복사 문자열
	...
	LPSTR lpCmdLine, int nCmdShow)
{
	...
	g_filesendmsg.type = TYPE_FILESEND;
	...
	case WM_COMMAND:
		...
		case IDC_SENDFILE:
			...
			if (GetOpenFileName(&ofn) != 0) {
				...
				HDC hdc = ::GetDC(hBtnSendFile);//핸들 얻음
...
DWORD WINAPI ReadThread(LPVOID arg)
{
	...
	while (1) {
		...
		if (comm_msg.type == TYPE_CHAT) {//채팅
			chat_msg = (CHAT_MSG*)&comm_msg;
			DisplayText("[받은 메시지] %s\r\n", chat_msg->msg);
		}
		else if (comm_msg.type == TYPE_DRAWLINE) {//선 그리기
		...
		else if (comm_msg.type == TYPE_ERASEPIC) {//그림 지우기
			erasepic_msg = (ERASEPIC_MSG*)&comm_msg;
			SendMessage(g_hDrawWnd, WM_ERASEPIC, 0, 0);
		}
		else if (comm_msg.type == TYPE_FILESEND) {//파일 전송
			filesend_msg = (FILESEND_MSG*)&comm_msg;
		}
	}
	return 0;
}
